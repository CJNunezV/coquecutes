...
async function ensureTable() {
  ...
  await sql`ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivery_info TEXT;`;
}
  ...
  try {
    ...
    try {
      ...
      const result = await sql`
        INSERT INTO orders (customer_name, customer_phone, items, total, screenshot_url, delivery_info)
        VALUES (${name}, ${phone}, ${JSON.stringify(items)}, ${total || 0}, ${screenshotUrl}, ${address || null})
        RETURNING id;
      `;
